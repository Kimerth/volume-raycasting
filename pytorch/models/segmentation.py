...
class UNet3D(nn.Module):
    def __init__(self, cfg: DictConfig):
        ...
        features = cfg["hparams"]["init_features"]

        self.encoders = [
            (
                UNet3D._block(cfg["hparams"]["in_channels"],
                              features, name="enc_1"),
                nn.MaxPool3d(kernel_size=2, stride=2)
            )
        ]
        for idx in range(1, cfg["hparams"]["nb_blocks"]):
            features *= 2
            self.encoders += [
                (
        ...
        self.decoders = []
        for idx in range(cfg["hparams"]["nb_blocks"], 0, -1):
            features /= 2
            self.decoders += [
                (
                    nn.ConvTranspose3d(features, features / 2,
                                       kernel_size=2, stride=2),
                    UNet3D._block(features, features / 8, name="dec_${idx}")
                )
            ]

        self.output_conv = nn.Conv3d(
            in_channels=features, out_channels=cfg["hparams"]["out_channels"], kernel_size=1)
    ...
    def forward(self, x):
        ...
        for idx, upconv, decoder in reversed(list(enumerate(self.decoders))):
            outputs['upconv_${idx}'] = torch.cat(
                (
                    upconv(get_last_output()),
                    outputs['enc_${idx}']
                ), dim=1)
            outputs['dec_${idx}'] = decoder(outputs['upconv_${idx}'])
